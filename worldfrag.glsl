...
float sphIntersect( in vec3 ro, in vec3 rd, in vec3 sph )
{
	vec3 oc = ro - sph.xyz;
	float b = dot( oc, rd );
	float c = dot( oc, oc ) - sph.y*sph.y;
	float h = b*b - c;
	if( h<0.0 ) return -1.0;
	return -b - sqrt( h );
}
...
vec3 render(vec3 ro, vec3 rd){
  vec3 light1 = normalize( vec3(2*radius,radius,0.f) );
    // bounding plane
    float tmin = 1.0;
    float tmax = 1000.0;
    //float tmax = 10000*tePosition.y; //minha Adap
#if 1
    float maxh = 1000.0*vcNormal.y;//*tePosition.y;//300.0*SC;
  ...
  if( t<=tmax)
    {
        // mountains
    vec3 fPos = vcPos;
		vec3 pos = (fPos*ro + t*rd);
    //vec3 nor = cross(calcNormal( pos, t ), vcNormal);
    vec3 nor = calcNormal( pos, t );
    vec3 ref = reflect( fPos*rd, nor );
    float fre = clamp( 1.0+dot(fPos*rd,nor), 0.0, 1.0 );

         //rock
		float r = noise((7.0)*pos.xz/256.0);
    col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08),
                                     noise(0.00007*vec2(pos.x,pos.y*48.0)));
		col = mix( col, 0.20*vec3(0.45,.30,0.15)*(0.50+0.50*r),smoothstep(0.70,0.9,nor.y) );
    col = mix( col, 0.15*vec3(0.30,.30,0.10)*(0.25+0.75*r),smoothstep(0.95,1.0,nor.y) );

		// snow
    float h = smoothstep(550.f,1000.0,pos.y + 25.0*vNoise ); //altura
    float e = smoothstep(1.0-0.5*h,1.0-0.1*h,nor.y);  //distribuição
    float o = 0.3 + vNoise*0.7*smoothstep(0.0,0.1,nor.x+h*h);
    float s = h*e*o;
...
void main(){
  ...
  vec2 xy = -1.0 + 2.0*gl_FragCoord.xy/(vec2(1280,1024));
